...
contract FurMarket is BaseContract, ERC721Holder
{
    ...
    mapping(address => uint256[]) _userListings;
    ...
    {
        _paymentToken = IERC20(addressBook.get("Tux"));
    }
     ...
     */
    function listNft(address tokenAddress_, uint256 tokenId_, uint256 price_) external
    {
        ...
        _userListings[msg.sender].push(_listingIdTracker);
     ...
     * @param listingId_ The ID of the listing.
     */
    function buyNft(uint256 listingId_) external
    {
        require(_listings[listingId_].start > 0, "Listing does not exist");
     ...
     */
    function makeOffer(uint256 listingId_, uint256 offer_) external
    {
        require(_listings[listingId_].start > 0, "Listing does not exist");
     ...
     * @param listingId_ The ID of the listing.
     */
    function rescindOffer(uint256 listingId_) external
    {
        require(_listings[listingId_].start > 0, "Listing does not exist");
     ...
     * @param listingId_ The ID of the listing.
     */
    function acceptOffer(uint256 listingId_) external
    {
        require(_listings[listingId_].start > 0, "Listing does not exist");
     ...
     * @param listingId_ The ID of the listing.
     */
    function rejectOffer(uint256 listingId_) external
    {
        require(_listings[listingId_].start > 0, "Listing does not exist");
